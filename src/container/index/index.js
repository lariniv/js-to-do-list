export class Todo {
  ...
  static #render = () => {
    ...
    if (this.#list.length === 0) {
      this.#block.innerText = 'Додайте задачу!'
    } else {
  ...
  static #createTaskElem = (data) => {
    ...
    // el.querySelector('.list__text').innerHTML = data.text

    // if (data.done) {
    //   el.querySelector('.list__button-do').className =
    //     'list__button list__button-done button'
    // }
    ...
    if (data.done) {
      el.classList.add('list__item--done')
      btnDo.classList.remove('list__button--do')
      btnDo.classList.add('list__button--done')
    }
  ...
  static #handleDo = (data, btn, el) => () => {
    ...
    if (result === true || result === false) {
      el.classList.toggle('list__item--done')
      btn.classList.toggle('list__button--do')
      btn.classList.toggle('list__button--done')
